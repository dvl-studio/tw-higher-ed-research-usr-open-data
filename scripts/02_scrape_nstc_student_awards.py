...
URL = "https://wsts.nstc.gov.tw/Award/AwardMultiQuery.aspx"
OUTPUT_DIR = Path(__file__).resolve().parent
OUTPUT_XLSX = OUTPUT_DIR / "02大專學生研究計畫_112-115.xlsx"
COOKIE_FILE = OUTPUT_DIR / "nstc_award_cookie.txt"
POST_FILE = OUTPUT_DIR / "nstc_award_post_body.txt"
USER_AGENT = (
...
PAGE_SIZE = "200"
CATEGORY_EVENT_TARGET = "dtlItem$ctl04$btnItem"
COLUMNS = [
    "年度",
    "學生姓名",
    "執行機關",
...
def select_student_project_category() -> str:
...
def query_year_range(page_html: str) -> str:
    fields, _ = parse_form_fields(page_html)
    fields.update(
        {
            "__EVENTTARGET": "",
            "__EVENTARGUMENT": "",
            "__LASTFOCUS": "",
            "wUctlAwardQueryPage$repQuery$ctl01$ddlYRst": min(YEARS),
            "wUctlAwardQueryPage$repQuery$ctl01$ddlYRend": max(YEARS),
...
def parse_rows(page_html: str) -> list[dict[str, str]]:
    ...
    for tr in doc.xpath('//table[@id="wUctlAwardQueryPage_grdResult"]/tr'):
        ...
        amount = re.sub(r"\D", "", content_value(cells[3], "核定金額"))
        rows.append(
            {
                "年度": year,
                "學生姓名": text_content(cells[1]),
                "執行機關": text_content(cells[2]),
                "計畫名稱": content_value(cells[3], "計畫名稱"),
                "計畫編號": content_value(cells[3], "計畫編號"),
                "執行起迄": content_value(cells[3], "執行起迄"),
                "指導教授": content_value(cells[3], "指導教授"),
                "核定金額": amount,
            }
        )
    return rows
...
def save_excel(rows: list[dict[str, str]]) -> None:
    ...
    df["核定金額"] = pd.to_numeric(df["核定金額"], errors="coerce").astype("Int64")
    df = df.drop_duplicates(subset=COLUMNS).sort_values(["年度", "執行機關", "學生姓名"])
    df.to_excel(OUTPUT_XLSX, index=False)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="下載 NSTC 大專學生研究計畫查詢結果。")
    parser.add_argument("--start-year", default=min(YEARS), help="起始年度，例如 110")
    parser.add_argument("--end-year", default=max(YEARS), help="結束年度，例如 115")
    parser.add_argument("--output", default=str(OUTPUT_XLSX), help="輸出的 xlsx 檔案路徑")
    return parser.parse_args()


def main() -> None:
    global OUTPUT_XLSX, YEARS
    ...
    YEARS = tuple(str(year) for year in range(start_year, end_year + 1))
    ...
    print("開啟大專學生研究計畫查詢頁...")
    category_html = select_student_project_category()
    first_page_html = query_year_range(category_html)
    pages = total_pages(first_page_html)
    rows = parse_rows(first_page_html)
    ...
    for page_index in range(1, pages):
        ...
        rows.extend(parse_rows(current_html))
